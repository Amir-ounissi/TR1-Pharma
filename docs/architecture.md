...
- La prochaine action est une projection commerciale unifiée : les `tasks` restent la source des tâches et `field_visits` la source des visites. `private.sync_brand_pharmacy_next_action` recalcule `next_action_type`, `next_action_at` et `next_action_owner_id` depuis l’action ouverte la plus proche, sans dupliquer une visite en tâche.
