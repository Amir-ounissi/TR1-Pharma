...
select set_config('request.jwt.claims','{"sub":"00000000-0000-0000-0000-0000000000a5","role":"authenticated"}',true);
select is((select count(*) from public.get_my_field_agenda('2030-06-10','2030-06-10',null) where source_id=(select id from missions where title='Proposition intervenant') and ownership='mine'),1::bigint,'approved mission is visible in provider agenda');
select set_config('request.jwt.claims','{"sub":"00000000-0000-0000-0000-0000000000a3","role":"authenticated"}',true);
select is((select count(*) from public.get_my_field_agenda('2030-06-10','2030-06-10',null) where source_id=(select id from missions where title='Proposition intervenant') and ownership='pharmacy_activity'),1::bigint,'approved mission is visible as pharmacy activity to responsible agent');
select set_config('request.jwt.claims','{"sub":"00000000-0000-0000-0000-0000000000a7","role":"authenticated"}',true);
select is((select count(*) from public.get_my_field_agenda('2030-06-10','2030-06-10',null) where source_id=(select id from missions where title='Proposition intervenant')),0::bigint,'unrelated agent cannot see pharmacy activity');
...
select is((select next_action_type from brand_pharmacies where id='00000000-0000-0000-0000-000000000411'),'follow_up','earlier open task keeps precedence over a later field visit');
...
select is((select count(*) from brand_pharmacies where id='00000000-0000-0000-0000-000000000414' and next_action_at='2030-06-12T13:00:00Z'::timestamptz),1::bigint,'rescheduling updates visit timing where the visit is the effective next action');
