...
begin
    ...
    and (
      ...
      or (
        ...
        and (
          ...
          or (
            length(private.normalize_reference_text(pharmacy.legal_name)) >= 8
            and (
              normalized_name like '%' || private.normalize_reference_text(pharmacy.legal_name) || '%'
              or private.normalize_reference_text(pharmacy.legal_name) like '%' || normalized_name || '%'
            )
          )
          or (
            length(private.normalize_reference_text(coalesce(pharmacy.trade_name, ''))) >= 8
            and (
              normalized_name like '%' || private.normalize_reference_text(coalesce(pharmacy.trade_name, '')) || '%'
              or private.normalize_reference_text(coalesce(pharmacy.trade_name, '')) like '%' || normalized_name || '%'
            )
          )
        )
      )
  ...
  order by
    case
      ...
      when normalized_name is not null and (
        private.normalize_reference_text(pharmacy.legal_name) = normalized_name
        or private.normalize_reference_text(coalesce(pharmacy.trade_name, '')) = normalized_name
      ) then 3
      when normalized_name is not null and length(normalized_name) >= 8 and (
        normalized_name like '%' || private.normalize_reference_text(pharmacy.legal_name) || '%'
        or private.normalize_reference_text(pharmacy.legal_name) like '%' || normalized_name || '%'
        or normalized_name like '%' || private.normalize_reference_text(coalesce(pharmacy.trade_name, '')) || '%'
        or private.normalize_reference_text(coalesce(pharmacy.trade_name, '')) like '%' || normalized_name || '%'
      ) then 4
      when normalized_postal_code is not null and regexp_replace(coalesce(pharmacy.postal_code, ''), '[^0-9]', '', 'g') = normalized_postal_code then 5
      else 6
    end,
