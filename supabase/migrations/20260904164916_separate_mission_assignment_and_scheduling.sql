...
begin
  ...
  if target.status <> 'accepted' then
    raise exception 'Only an accepted mission can be scheduled' using errcode='23514';
  end if;

  if target_scheduled_start_at is null then
    raise exception 'A start date is required' using errcode='23514';
  end if;

  if target_scheduled_end_at is not null and target_scheduled_end_at < target_scheduled_start_at then
...
revoke all on function public.schedule_mission(uuid,timestamptz,timestamptz) from public;
