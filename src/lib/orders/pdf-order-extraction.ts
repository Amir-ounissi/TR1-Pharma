...
export async function extractPdfOrder(file: File, fetcher: Fetcher = fetch): Promise<PdfOrderExtraction> {
  ...
  try {
    ...
    model: process.env.OPENAI_PDF_ORDER_MODEL ?? "gpt-5",
        store: false,
        instructions: "Extract only the fields requested from this purchase-order PDF. The pharmacy object MUST describe the buying/customer/delivery pharmacy (the officine placing or receiving the order), never the supplier, manufacturer or brand. Labels such as Fournisseur identify the supplier and must not be used as the pharmacy. Never infer CIP, SIRET or FINESS from an unlabeled number: populate those identifiers only when they are explicitly labeled or unambiguous. For orderDate, return YYYY-MM-DD. If an explicit Date de Commande field is blank but the document has a clearly printed header/document date such as 'Le 31/08/2026', use that date as orderDate. For each product row, quantity means the paid/ordered quantity (for example Qté Cmde) and freeQuantity means free units (for example Qté UG, UG or gratuité). Never convert free units into a 100% commercial discount: if a row contains only free units, set quantity to null and freeQuantity to that number. discountRate describes only a discount on paid units. Never infer a TR1 identifier, never perform matching, preserve the pharmacy name as printed, and use null when a value is absent or unreadable.",
