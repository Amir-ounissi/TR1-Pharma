...
const pharmacies = [
  { id: "pharmacy-1", name: "Pharmacie du Centre", siret: "12345678901234", cip: "CIP-1", finess: "FIN-1", postalCode: "75001" },
  { id: "pharmacy-2", name: "Pharmacie du Centre", siret: "999", cip: "CIP-2", finess: "FIN-2", postalCode: "75002" },
];
const products = [
...
describe("PDF order deterministic matching", () => {
  it("matches a pharmacy by SIRET before other information", () => {
    expect(matchPdfPharmacy({ name: "Other", siret: "123 456 789 01234", cip: null, finess: null, address: null, postalCode: null }, pharmacies)).toMatchObject({ status: "matched", method: "siret", match: { id: "pharmacy-1" } });
  });

  it("matches a pharmacy by CIP or FINESS", () => {
    expect(matchPdfPharmacy({ name: null, siret: null, cip: "cip-2", finess: null, address: null, postalCode: null }, pharmacies)).toMatchObject({ status: "matched", method: "cip", match: { id: "pharmacy-2" } });
    expect(matchPdfPharmacy({ name: null, siret: null, cip: null, finess: "fin-1", address: null, postalCode: null }, pharmacies)).toMatchObject({ status: "matched", method: "finess", match: { id: "pharmacy-1" } });
  });

  it("requires manual selection when a pharmacy match is ambiguous", () => {
    expect(matchPdfPharmacy({ name: "Pharmacie du Centre", siret: null, cip: null, finess: null, address: null, postalCode: "75001" }, [{ ...pharmacies[0] }, { ...pharmacies[0], id: "duplicate" }])).toMatchObject({ status: "ambiguous", match: null });
  });
