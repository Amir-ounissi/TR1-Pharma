import { legalInformation, missingLegalInformation, temporaryLegalInformation } from "@/config/legal";
...
export default function PrivacyPage() {
  const sections = [
    ...
    ["Durée de conservation", legalInformation.leadRetentionDuration],
  ...
  const showTemporaryNotice = temporaryLegalInformation || missingLegalInformation.length > 0;
  return <main className="mx-auto min-h-[70vh] max-w-3xl px-5 py-16"><p className="font-mono text-xs font-bold uppercase tracking-[.16em] text-[#c9562d]">Données personnelles</p><h1 className="mt-3 text-4xl font-black tracking-[-.05em]">Politique de confidentialité</h1>{showTemporaryNotice ? <p className="mt-6 rounded-lg border border-[#c9562d] bg-[#fff6ec] p-4 text-sm font-semibold text-[#7a351d]">Version temporaire de pré-lancement — les informations administratives définitives seront publiées après validation. La collecte de leads est désactivée pendant cette phase.</p> : null}<div className="mt-8 space-y-8">{sections.map(([title,content]) => <section key={title}><h2 className="font-mono text-sm font-black uppercase tracking-[.05em]">{title}</h2><p className={`mt-2 leading-7 ${content.startsWith("À renseigner") ? "text-[#c9562d]" : "text-[#596574]"}`}>{content}</p></section>)}</div><p className="mt-10 text-xs text-[#66717d]">Dernière mise à jour : {legalInformation.privacyPolicyUpdatedAt}</p></main>;
}
