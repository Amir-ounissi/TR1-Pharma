...
export async function createProductAction(_state: ReferenceActionState, formData: FormData): Promise<ReferenceActionState> {
  const schema = z.object({ name: z.string().trim().min(2), sku: z.string().trim().min(1), ean: optionalText, category: optionalText, format: optionalText, productFamily: optionalText, strategicPriority: z.enum(["standard","priority","strategic"]), pharmacyEligible: z.boolean(), countsForDistribution: z.boolean(), wholesalePrice: z.union([z.coerce.number().min(0), z.literal("")]), retailPrice: z.union([z.coerce.number().min(0), z.literal("")]) });
  const parsed = schema.safeParse({ name: formData.get("name"), sku: formData.get("sku"), ean: formData.get("ean"), category: formData.get("category"), format: formData.get("format"), productFamily: formData.get("productFamily"), strategicPriority: formData.get("strategicPriority"), pharmacyEligible: formData.get("pharmacyEligible") === "on", countsForDistribution: formData.get("countsForDistribution") === "on", wholesalePrice: formData.get("wholesalePrice"), retailPrice: formData.get("retailPrice") });
  if (!parsed.success) return { error: "Produit invalide." };
  const { supabase, brand } = await requireActiveBrand();
  const { error } = await supabase.from("products").insert({ brand_id: brand.id, name: parsed.data.name, sku: parsed.data.sku, ean: parsed.data.ean || null, category: parsed.data.category || null, format: parsed.data.format || null, product_family: parsed.data.productFamily || null, strategic_priority: parsed.data.strategicPriority, is_pharmacy_eligible: parsed.data.pharmacyEligible, counts_for_distribution: parsed.data.countsForDistribution, wholesale_price_ht: parsed.data.wholesalePrice === "" ? null : parsed.data.wholesalePrice, retail_price_ttc: parsed.data.retailPrice === "" ? null : parsed.data.retailPrice });
