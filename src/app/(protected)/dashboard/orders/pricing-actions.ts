...
async function editableBrandPharmacy(pharmacyId: string) {
  ...
  if (!role || !allowedRoles.has(role)) throw new Error("Votre rôle ne permet pas de modifier les conditions commerciales.");
  ...
  const { data: relation, error } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  if (error || !relation) throw new Error("Cette pharmacie n’est pas rattachée à la marque active.");
...
export async function saveOrderPharmacyCommercialTermsAction(input: {
  pharmacyId: string;
  ...
  const parsed = z.object({
    ...
    if ((value.ugPaidQuantity === null) !== (value.ugFreeQuantity === null)) {
      ctx.addIssue({ code: "custom", message: "La base UG et la quantité offerte doivent être renseignées ensemble." });
    }
  }).parse(input);
...
export async function resetOrderPharmacyCommercialTermsAction(pharmacyId: string) {
  const parsedPharmacyId = uuid.parse(pharmacyId);
