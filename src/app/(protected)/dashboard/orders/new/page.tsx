...
export default async function NewOrderPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div className="space-y-6"><div><h1 className="text-2xl font-semibold">Nouvelle commande</h1><p className="text-muted-foreground">Les snapshots et totaux sont figés et recalculés côté serveur.</p></div><Card><CardHeader><CardTitle>Commande</CardTitle></CardHeader><CardContent><OrderEntryModes manual={<OrderForm products={productOptions} initialPharmacy={initialPharmacy} isAgent={isAgent} />} /></CardContent></Card></div>;
}
