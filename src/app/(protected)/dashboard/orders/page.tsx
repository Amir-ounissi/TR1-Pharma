...
export default async function OrdersPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const previousHref = buildPageHref("/dashboard/orders", params, page - 1);
  const nextHref = buildPageHref("/dashboard/orders", params, page + 1);
  return <div className="space-y-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-2xl font-semibold">{isAgent ? "Mes commandes" : "Commandes"}</h1><p className="text-muted-foreground">{isAgent ? "Suivez les commandes envoyées à la marque et les éventuelles corrections demandées." : canOperate ? `Validez les commandes terrain puis suivez leur facturation et leur livraison pour ${brand.name}.` : `Consultez les commandes et leur avancement pour ${brand.name}.`}</p></div><div className="flex gap-2">{canImport ? <Button asChild variant="outline"><Link href="/dashboard/imports">Importer un CSV</Link></Button> : null}{canOperate ? <Button asChild><Link href="/dashboard/orders/new">Nouvelle commande</Link></Button> : null}</div></div>
    ...
    <Card><CardContent className="p-0">{error ? <p className="p-6 text-destructive">Chargement impossible.</p> : !orders?.length ? <div className="p-10 text-center"><p className="font-medium">Aucune commande enregistrée.</p>{canImport || canOperate ? <Button asChild className="mt-4" size="sm"><Link href={canImport ? "/dashboard/imports" : "/dashboard/orders/new"}>{canImport ? "Importer des commandes" : "Créer une commande"}</Link></Button> : null}</div> : <Table><TableHeader><TableRow><TableHead>Commande</TableHead><TableHead>Pharmacie</TableHead><TableHead>Date</TableHead><TableHead>Type</TableHead><TableHead>Statut</TableHead><TableHead>Net HT</TableHead>{canViewPayment ? <TableHead>Règlement</TableHead> : null}</TableRow></TableHeader><TableBody>{orders.map((order) => { const pharmacy = Array.isArray(order.pharmacies) ? order.pharmacies[0] : order.pharmacies; return <TableRow key={order.id}><TableCell><Link href={`/dashboard/orders/${order.id}`} className="font-medium hover:underline">{order.order_number || order.external_order_id || order.id.slice(0, 8)}</Link></TableCell><TableCell>{pharmacy?.trade_name || pharmacy?.legal_name}<p className="text-xs text-muted-foreground">{pharmacy?.city}</p></TableCell><TableCell>{new Date(order.order_date).toLocaleDateString("fr-FR")}</TableCell><TableCell>{order.is_initial_order ? "Implantation" : order.is_reorder ? "Réassort" : uiLabel(order.order_type)}</TableCell><TableCell><Badge variant="secondary">{orderStatusLabel(order.order_status)}</Badge></TableCell><TableCell>{formatCurrency(order.net_amount_ht)}</TableCell>{canViewPayment ? <TableCell>{order.payment_status === "not_applicable" ? "Non connecté" : uiLabel(order.payment_status)}</TableCell> : null}</TableRow>; })}</TableBody></Table>}</CardContent></Card>
    <div className="flex items-center justify-between"><p className="text-sm text-muted-foreground">{count ?? 0} commande(s)</p><div className="flex gap-2"><Button asChild variant="outline" disabled={page <= 1}><Link href={previousHref}>Précédent</Link></Button><span className="self-center text-sm">{page}/{totalPages}</span><Button asChild variant="outline" disabled={page >= totalPages}><Link href={nextHref}>Suivant</Link></Button></div></div>
  </div>;
}
