...
export default async function OrdersPage({ searchParams }: { searchParams: SearchParams }) {
  const [params, session, contexts] = await Promise.all([
    searchParams,
    requireActiveBrand(),
    getBrandContexts(),
  ]);
  ...
  const workflowSummaryPromise = supabase.rpc("get_order_workflow_summary", {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold">{isAgent ? "Mes commandes" : "Commandes"}</h1>
          <p className="text-muted-foreground">
            {isAgent
              ? "Suivez les commandes envoyées à la marque et les éventuelles corrections demandées."
              : canOperate
                ? `Validez les commandes terrain puis suivez leur facturation et leur livraison pour ${brand.name}.`
                : `Consultez les commandes et leur avancement pour ${brand.name}.`}
          </p>
        </div>
        <div className="flex gap-2">
          {canImport ? <Button asChild variant="outline"><Link href="/dashboard/imports">Importer un CSV</Link></Button> : null}
          {canOperate ? <Button asChild><Link href="/dashboard/orders/new">Nouvelle commande</Link></Button> : null}
        </div>
      </div>
      ...
      <Card>
        <CardContent className="p-0">
          {error ? (
            <p className="p-6 text-destructive">Chargement impossible.</p>
          ) : !orders?.length ? (
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">{count ?? 0} commande(s)</p>
        <div className="flex gap-2">
          <Button asChild variant="outline" disabled={page <= 1}><Link href={previousHref}>Précédent</Link></Button>
          <span className="self-center text-sm">{page}/{totalPages}</span>
          <Button asChild variant="outline" disabled={page >= totalPages}><Link href={nextHref}>Suivant</Link></Button>
        </div>
      </div>
    </div>
  );
