...
export async function GET(_request: Request, { params }: RouteContext) {
  ...
  const { data: order, error: orderError } = await supabase
    .from("orders")
    .select(
      "id,brand_id,pharmacy_id,order_number,external_order_id,order_date,subtotal_ht,discount_amount_ht,net_amount_ht,tax_amount,total_ttc,notes,created_by",
    )
    .eq("id", id)
  ...
  const [
    ...
    supabase
      .from("brands")
      .select("name,code,order_email")
      .eq("id", brand.id)
      .single(),
    supabase
      .from("pharmacies")
      .select(
        "legal_name,trade_name,cip_code,siret,vat_number,email,phone,address_line_1,address_line_2,postal_code,city",
      )
    ...
    supabase
      .from("order_items")
      .select(
        "product_id,product_name_snapshot,sku_snapshot,quantity,free_quantity,unit_price_ht,discount_rate,net_unit_price_ht,line_total_ht,tax_rate",
      )
    ...
    admin
      .from("users")
      .select("email")
      .eq("id", order.created_by || userId)
      .maybeSingle(),
  ]);

  if (brandError || pharmacyError || itemsError || !brandData || !pharmacy) {
    return new Response("Impossible de préparer le bon de commande.", {
      status: 500,
    });
  }
  ...
  if (!(items ?? []).length) {
    return new Response(
      "Le bon de commande ne peut pas être généré sans ligne produit.",
      { status: 422 },
    );
  }
  ...
  if (productsError) {
    return new Response("Impossible de charger le référentiel produits.", {
      status: 500,
    });
  }
  ...
  const reference =
    order.order_number || order.external_order_id || order.id.slice(0, 8);
  ...
  const filename = `bon-de-commande-${safeFileName(reference)}.pdf`;
