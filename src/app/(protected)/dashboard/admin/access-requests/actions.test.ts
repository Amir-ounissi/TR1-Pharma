...
describe("access request review actions", () => {
  ...
  it("approves a brand only on the explicit matched brand", async () => {
    ...
    mocks.requirePlatformAdmin.mockResolvedValue({
      supabase: approvedBrandRequestClient(rpc),
    });
  ...
  it("approves an agent with explicit departments", async () => {
    const rpc = vi.fn(async () => ({ error: null }));
    ...
    formData.set("requestId", "00000000-0000-4000-8000-000000000001");
    formData.set("targetBrandId", "00000000-0000-4000-8000-000000000002");
    formData.append("departmentCodes", "13");
    formData.append("departmentCodes", "30");
    formData.append("departmentCodes", "34");

    const result = await approveAgentAccessRequestAction({}, formData);

    expect(result.success).toMatch(/3 départements/i);
    expect(rpc).toHaveBeenCalledWith(
      "approve_access_request_with_departments",
      {
        target_request_id: "00000000-0000-4000-8000-000000000001",
        target_brand_id: "00000000-0000-4000-8000-000000000002",
        target_department_codes: ["13", "30", "34"],
        review_note: null,
      },
    );
  });
  ...
  it("deduplicates selected departments", async () => {
    ...
    await approveAgentAccessRequestAction({}, formData);

    expect(rpc).toHaveBeenCalledWith(
      "approve_access_request_with_departments",
      expect.objectContaining({
        target_department_codes: ["34"],
      }),
    );
  });

  it("rejects an agent approval without a department before calling the RPC", async () => {
    const rpc = vi.fn(async () => ({ error: null }));
    ...
    expect(result.error).toMatch(/département/i);
    expect(mocks.requirePlatformAdmin).not.toHaveBeenCalled();
    expect(rpc).not.toHaveBeenCalled();
  });

  it("rejects an invalid department before calling the RPC", async () => {
    const rpc = vi.fn(async () => ({ error: null }));

    const formData = new FormData();
    formData.set("requestId", "00000000-0000-4000-8000-000000000001");
    formData.set("targetBrandId", "00000000-0000-4000-8000-000000000002");
    formData.set("departmentCodes", "99");
            ...
            eq: vi.fn(() => ({
              eq: vi.fn(() => ({
                maybeSingle: vi.fn(async () => ({
                  data: { requested_profile_type: "agent" },
                  error: null,
                })),
              })),
            })),
