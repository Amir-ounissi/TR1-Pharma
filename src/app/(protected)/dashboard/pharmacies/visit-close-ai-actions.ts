...
import { z } from "zod";
import { completeVisitAction } from "@/app/(protected)/dashboard/pharmacies/quick-actions";
import { analyzeVisitCloseNote, type VisitCloseDraft } from "@/lib/assistant/visit-close-ai";
import { requireActiveBrand } from "@/lib/auth";

const uuid = z.string().uuid();
const finishOutcome = z.enum(["order_taken", "no_order", "follow_up", "information", "other"]);
...
export async function getVisitCloseAvailabilityAction(brandPharmacyId: string): Promise<VisitCloseAvailability> {
  try {
    ...
    if (message === "Aucune visite à clôturer pour cette pharmacie.") return { active: false };
...
export async function completeVisitWithAssistantAction(input: {
  brandPharmacyId: string;
  visitId: string;
  outcome: "order_taken" | "no_order" | "follow_up" | "information" | "other";
  ...
  try {
    ...
    // Re-check ownership/state immediately before persistence. The canonical
    // closeout RPC remains the final authorization + idempotency boundary.
    await requireOwnedVisit(parsed.brandPharmacyId, parsed.visitId);

    return await completeVisitAction(
