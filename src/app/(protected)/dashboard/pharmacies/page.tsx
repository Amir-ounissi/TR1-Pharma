...
export default async function PharmaciesPage({
  ...
  if (attentionOnly) {
    query = query.in("activity_status", ["at_risk", "dormant"]);
  }
  ...
  const requestedSort =
    typeof params.sort === "string" ? params.sort : "trade_name";
  const sort = sortableColumns.includes(
    requestedSort as (typeof sortableColumns)[number],
  )
    ? requestedSort
    : "trade_name";
  ...
  const strategicCount = rows.filter(
    (row) => row.priority_level === "strategic",
  ).length;
  ...
  const hasActiveFilters = [
    ...
    (key) =>
      ...
      params[key] !== "" &&
      params[key] !== "all" &&
      !(key === "sort" && params[key] === "trade_name") &&
      !(key === "direction" && params[key] === "asc") &&
      !(key === "page" && params[key] === "1"),
  );
  ...
  return (
    <div
      className={
        view === "map"
          ? "flex min-h-[calc(100vh-8.5rem)] flex-col gap-3 overflow-hidden"
          : "space-y-3"
      }
    >
      {view === "list" ? (
        <>
            ...
            <CompactPageHeader
              ...
              actions={
                role !== "agent" ? (
                  <Button
                    asChild
                    className="h-9 rounded-md bg-[var(--tr1-navy)] px-3.5 text-sm font-medium text-white hover:bg-[var(--tr1-navy-soft)]"
                  >
          ...
          <div className="flex items-end justify-between gap-3 md:hidden">
            ...
            >
              <Link href={buildViewHref(urlParams, "map")}>
      ...
      <div className={view === "list" ? "hidden md:block" : "block"}>
        <MapFilters
          ...
          mode={mapDataset?.mode ?? "network"}
          params={urlParams}
          period={mapDataset?.period ?? "30d"}
          roleScope={roleScope}
          view={view}
        />
      </div>
        ...
        <div className="hidden md:block">
          <MetricStrip
            items={[
              {
                icon: Building2,
                label: role === "agent" ? "Mon portefeuille" : "Portefeuille total",
                value: count ?? 0,
                detail: "Pharmacies référencées",
              },
              {
                icon: CircleAlert,
                label: "Priorités",
              ...
              role === "agent"
                ...
                : {
                    ...
                    detail: "Sur cette page · affectation à compléter",
                  },
              {
                icon: MapPin,
      ...
      {view === "list" ? (
        <>
                      ...
                      {role !== "agent" ? (
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <Input
                            name="territory"
                            defaultValue={
                              typeof params.territory === "string"
                                ? params.territory
                                : ""
                            }
                            className="h-11 rounded-xl bg-white text-sm"
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <Select name="sort" defaultValue={sort}>
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="commercial_status">
                              Statut
                            </SelectItem>
                            <SelectItem value="priority_level">
                              Priorité
                            </SelectItem>
                            <SelectItem value="potential_level">
                              Potentiel
                            </SelectItem>
                          </SelectContent>
                        </Select>
                        <Select
                          name="direction"
                          defaultValue={descending ? "desc" : "asc"}
                        >
                          <SelectTrigger className="h-11 w-full rounded-xl bg-white">
                            <SelectValue />
                          </SelectTrigger>
              ...
              <ToolbarRow className="justify-between">
                ...
                {hasActiveFilters ? (
                  <Button
                    ...
                    size="sm"
                    variant="ghost"
                    className="h-8 px-2.5 text-[var(--tr1-navy)]"
                  >
                    <Link href="/dashboard/pharmacies?view=list">
              ...
              <form className="mt-2 flex flex-col gap-2.5 lg:flex-row lg:flex-nowrap lg:items-center">
                <div className="relative lg:min-w-0 lg:flex-1">
                  ...
                  <Input
                    name="q"
                    defaultValue={search}
                    className="h-9 rounded-md bg-white/90 pl-9 text-sm"
                    placeholder="Rechercher une pharmacie, une ville, un CIP…"
                  />
                </div>
                <Select
                  name="status"
                ...
                >
                  <SelectTrigger className="h-9 w-full rounded-md bg-white/90 lg:w-[11rem] lg:min-w-[11rem]">
                    <SelectValue placeholder="Statut" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">Tous les statuts</SelectItem>
                ...
                <input
                  type="hidden"
                  name="agent"
                  value={typeof params.agent === "string" ? params.agent : ""}
                />
                <input
                  type="hidden"
                  name="territory"
                ...
                <input
                  type="hidden"
                  name="group"
                  value={typeof params.group === "string" ? params.group : ""}
                />
                ...
                <Sheet>
                  <SheetTrigger asChild>
                    <Button
                      type="button"
                      variant="outline"
                      className="h-9 rounded-md border-[var(--tr1-line-strong)] bg-white/80 px-3 text-sm text-[var(--tr1-navy)] lg:w-[8.25rem] lg:min-w-[8.25rem]"
                    >
                      <SlidersHorizontal className="size-3.5" />
                      Filtres +
                    </Button>
                    ...
                    <form className="contents">
                      ...
                      <div className="mt-6 grid gap-3 sm:grid-cols-2">
                        ...
                        <Input
                          ...
                          className="h-10 rounded-md bg-white text-sm sm:col-span-2"
                          placeholder="Groupement"
                        />
                        <Select name="sort" defaultValue={sort}>
                          <SelectTrigger className="h-10 w-full rounded-md bg-white">
                        ...
                        <Select
                          ...
                          defaultValue={descending ? "desc" : "asc"}
                        >
                          <SelectTrigger className="h-10 w-full rounded-md bg-white sm:col-span-2">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="asc">Tri croissant</SelectItem>
                            <SelectItem value="desc">Tri décroissant</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                      <div className="mt-5 flex items-center justify-between gap-2">
                        <Button
                          ...
                          type="button"
                          variant="ghost"
                          className="h-9 px-0 text-[var(--tr1-navy)] hover:bg-transparent hover:text-[var(--tr1-orange)]"
                        >
                          <Link href="/dashboard/pharmacies?view=list">
                            Réinitialiser
                          </Link>
                ...
                <Button
                  type="submit"
                  className="h-9 rounded-md bg-[var(--tr1-navy)] px-3 text-sm font-medium text-white hover:bg-[var(--tr1-navy-soft)] lg:min-w-[6.5rem]"
                >
                  Appliquer
                </Button>
              </form>
          ...
          {error ? (
            ...
            <EmptyState
              ...
              title={
                hasActiveFilters
                  ...
                  : "Aucune pharmacie ajoutée."
              }
              description={
                hasActiveFilters
                  ? "Essayez d’élargir votre recherche ou de réinitialiser les filtres."
                  : role === "agent"
                    ? "Votre portefeuille ne comporte pas encore de pharmacie accessible."
              ...
              action={
                hasActiveFilters ? (
                  ...
                  <Button
                    asChild
                    size="sm"
                    className="h-9 bg-[var(--tr1-navy)] px-3.5 text-sm font-medium text-white hover:bg-[var(--tr1-navy-soft)]"
                  >
                    <Link href="/dashboard/pharmacies/new">
            ...
            <PharmacyListWithPanel
              rows={rows}
              loadSummaryAction={loadPharmacySummaryAction}
            />
          )}
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={page <= 1}
                >
                  <Link href={buildPageHref(urlParams, Math.max(1, page - 1))}>
                    Précédent
                  </Link>
                </Button>
                <Button
...
function buildViewHref(current: URLSearchParams, view: NetworkMapView) {
  const next = new URLSearchParams(current.toString());
  next.set("view", view);
  next.delete("page");
  return `?${next.toString()}`;
