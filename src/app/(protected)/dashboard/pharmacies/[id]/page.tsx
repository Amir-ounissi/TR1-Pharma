...
export default async function PharmacyDetailPage({
  ...
  const canCreateContacts =
    canManageAccount ||
    (isAgent && Boolean(settings?.allow_agents_to_create_contacts));

  const canEditPotential =
    canManageAccount ||
    (isAgent && Boolean(settings?.allow_agents_to_edit_potential));

  const canChangeStatus =
    canManageAccount ||
    (isAgent && Boolean(settings?.allow_agents_to_change_status));
  ...
  const cockpit = getPharmacyCockpit({
    ...
    strategicDistributionRate: distribution?.strategic_distribution_rate,
    missingProducts,
