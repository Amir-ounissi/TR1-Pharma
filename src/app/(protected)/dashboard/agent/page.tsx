...
import { VisitDayProgress } from "@/components/agent/visit-day-progress";
...
import type { FieldVisitAgendaEvent, FieldVisitCompletion } from "@/lib/field-visit-progress";
...
export default async function AgentPage() {
  ...
  const [{ data: agenda }, { data: nextVisit }, opportunitiesResult, recentImpactResult, fieldAgendaResult] = await Promise.all([
    ...
    saas.capabilities.has("missions")
      ...
      : Promise.resolve({ data: [] }),
    supabase.rpc("get_my_field_agenda", { start_date: today, end_date: today, brand_filter: brand.id }),
  ]);

  const fieldVisits = ((fieldAgendaResult.data ?? []) as FieldVisitAgendaEvent[]).filter(
    (event) => event.source_kind === "field_visit" && event.ownership === "mine",
  );
  const visitIds = [...new Set(fieldVisits.map((visit) => visit.source_id))];
  const completionResult = visitIds.length
    ? await supabase
        .from("field_visit_completions")
        .select("visit_id,brand_id,brand_pharmacy_id,order_result,photo_result,note,next_visit_date,completed_at")
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-5 pb-[calc(2rem+env(safe-area-inset-bottom))]">
      ...
      <VisitDayProgress
        ...
        today={today}
        visits={fieldVisits}
        completions={(completionResult.data ?? []) as FieldVisitCompletion[]}
      />
