...
export function TerrainPharmacyHeader(props: TerrainPharmacyHeaderProps) {
  ...
  const noteVisitId = resolvedVisit?.id ?? "";
  ...
  function resolvePrimaryVisitAction() {
    if (resolvedVisit) {
      router.push(`/dashboard/visits/${resolvedVisit.id}`);
      return;
    }
    startTransition(async () => {
      ...
      <div className="space-y-4 p-4 sm:p-6">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          {canQuickVisit ? (
            ...
            >
              {resolvedVisit ? <ClipboardPenLine className="size-5" /> : <CalendarPlus className="size-5" />}
              {visitLabel}
            </Button>
          ) : (
