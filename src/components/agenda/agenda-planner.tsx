...
const filters = [
  { key: "all", label: "Tout" },
  { key: "mine", label: "Mon planning" },
  { key: "pharmacy_activity", label: "Dans mes pharmacies" },
...
export function AgendaPlanner({
  ...
  const visible = events.filter(
    (event) =>
      filter === "all" ||
      event.ownership === filter ||
      event.source_kind === filter,
  );
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="font-mono text-[0.65rem] font-bold uppercase tracking-[0.16em] text-[var(--tr1-orange)]">
            Agenda Terrain · toutes vos marques
          </p>
          <h1 className="mt-1 text-3xl font-black tracking-tight text-[var(--tr1-navy)]">
            Organisez votre terrain
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Une seule vue pour vos visites, missions, tâches et activités en
            pharmacie.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          {canCreateVisit ? <VisitSheet pharmacies={pharmacies} /> : null}
          <BlockSheet />
        </div>
      </header>
      <div className="grid gap-4 xl:grid-cols-[13rem_minmax(0,1fr)_17rem]">
        <aside className="space-y-4 rounded-xl border border-[var(--tr1-line)] bg-white/70 p-3">
          ...
          <div className="grid grid-cols-2 gap-1">
            <Button
              ...
              onClick={() =>
                router.push(`/dashboard/agenda?date=${date}&view=day`)
              }
            >
              Jour
            </Button>
            <Button
              ...
              onClick={() =>
                router.push(`/dashboard/agenda?date=${date}&view=week`)
              }
            >
              Semaine
            </Button>
        ...
        <main className="min-w-0 overflow-hidden rounded-xl border border-[var(--tr1-line)] bg-white/80">
          <div className="flex items-center justify-between border-b p-3">
            ...
            <div className="flex items-center gap-2 text-sm font-bold text-[var(--tr1-navy)]">
              <CalendarDays className="size-4" />
              {view === "week"
                ? `Semaine du ${new Date(`${date}T12:00:00`).toLocaleDateString("fr-FR")}`
          ...
          <div
            className={cn(
              "grid min-w-[44rem]",
              view === "week"
                ? "grid-cols-[3rem_repeat(7,minmax(7rem,1fr))]"
                : "grid-cols-[3.5rem_minmax(19rem,1fr)_minmax(14rem,0.42fr)]",
            )}
          >
            <div />
            <>
              {days.map((day) => (
                <div
                  className="border-l p-2 text-center text-xs font-bold"
                  key={day}
                >
            ...
            {view === "day" ? (
              <div className="border-l p-2 text-center text-xs font-bold text-muted-foreground">
                Dans mes pharmacies
              </div>
            ) : null}
            {hours.map((hour) => (
        ...
        <aside className="rounded-xl border border-[var(--tr1-line)] bg-white/70 p-3">
          <h2 className="font-bold text-[var(--tr1-navy)]">À planifier</h2>
          <p className="mb-3 text-xs text-muted-foreground">
            Tâches ouvertes, retards et corrections.
          </p>
          <div className="space-y-2">
            {backlog.map((item) => (
              <Link
                href={item.detail_url}
                className="block rounded-lg border bg-white p-3 text-sm hover:border-[var(--tr1-orange)]"
                key={item.item_key}
              >
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {item.pharmacy_name} · {item.brand_name}
                </p>
              </Link>
            ))}
            {!backlog.length ? (
  ...
  const cells = days.map((day) =>
    events.filter(
      (event) =>
        ...
        isoToParisLocal(event.start_at).startsWith(
          `${day}T${String(hour).padStart(2, "0")}`,
        ),
    ),
  );
  const pharmacy = events.filter(
    (event) =>
      event.ownership === "pharmacy_activity" &&
      isoToParisLocal(event.start_at).startsWith(
        `${days[0]}T${String(hour).padStart(2, "0")}`,
      ),
  );
  return (
    <>
      <div className="border-t p-2 text-right font-mono text-[0.62rem] text-muted-foreground">
        {hour}:00
      </div>
      {cells.map((items, index) => (
        <div
          className="min-h-20 border-l border-t p-1.5"
          onDragOver={(event) => event.preventDefault()}
          onDrop={(event) => onDrop(event, days[index], hour)}
          key={days[index]}
        >
          {items.map((item) => (
            <EventCard event={item} key={item.event_key} />
          ))}
        </div>
      ))}
      {view === "day" ? (
        <div className="min-h-20 border-l border-t bg-slate-50/70 p-1.5">
          {pharmacy.map((item) => (
            <EventCard event={item} key={item.event_key} />
          ))}
        </div>
...
function EventCard({ event }: { event: AgendaEvent }) {
  ...
  return (
    <Link
      ...
      className={cn(
        "mb-1 block rounded-md border-l-4 p-2 text-xs shadow-sm",
        tone,
      )}
    >
      <div className="flex items-start gap-1">
        {event.draggable ? (
          <GripVertical className="mt-0.5 size-3 shrink-0" />
        ) : null}
        <strong className="line-clamp-2">{event.title}</strong>
      </div>
      <p className="mt-1 text-[0.65rem] text-muted-foreground">
        <Clock3 className="mr-1 inline size-3" />
        {isoToParisLocal(event.start_at).slice(11)} ·{" "}
        {event.pharmacy_name || uiLabel(event.status)}
      </p>
      {event.ownership === "pharmacy_activity" ? (
        <p className="mt-1 font-medium">
          Lecture seule · {event.assigned_user_name || "Intervenant"}
        </p>
      ) : null}
    </Link>
  );
      ...
      <SheetContent className="overflow-y-auto">
        ...
        <form action={action} className="space-y-4 p-4">
          ...
          <Field label="Marques concernées">
            <div className="space-y-2">
              {selected?.brands.map((brand) => (
                <label className="flex gap-2 text-sm" key={brand.relationId}>
                  <input
                    type="checkbox"
          ...
          <Button disabled={pending} className="w-full">
            {pending ? "Création…" : "Confirmer la visite"}
          </Button>
        </form>
      </SheetContent>
    </Sheet>
