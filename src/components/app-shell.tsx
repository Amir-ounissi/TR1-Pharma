    ...
    <div className="tr1-product-da min-h-screen bg-[var(--tr1-ivory)]">
      ...
      <div className="md:pl-[16.5rem]">
        <header data-testid="mobile-sticky-header" className="sticky top-0 z-30 flex min-h-[4.25rem] items-center gap-3 border-b border-[var(--tr1-line)] bg-[var(--tr1-ivory)]/94 px-3 backdrop-blur-xl sm:px-5" style={{ paddingTop: "env(safe-area-inset-top)" }}>
          ...
          {showPlatformAdministrationReturn ? (
            <form action={returnToPlatformAdministrationAction} className="shrink-0">
              <Button className="h-10 gap-2 px-2.5" title="Retour Administration TR1" type="submit" variant="outline">
                <ShieldCheck className="size-4 text-[var(--tr1-orange)]" />
                <span className="hidden sm:inline">Retour Administration TR1</span>
                <span className="sm:hidden">Admin TR1</span>
              </Button>
            </form>
          ) : (
...
function PlatformAdministrationReturn() {
  return (
    <form action={returnToPlatformAdministrationAction}>
      <Button className="w-full justify-start border-white/15 bg-white/10 text-white hover:bg-white/15 hover:text-white" type="submit" variant="outline">
        <ShieldCheck className="size-4 text-[var(--tr1-orange)]" />
        Retour Administration TR1
      </Button>
    </form>
  );
}
