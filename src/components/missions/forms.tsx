...
export function MissionForm({ pharmacies, users, products }: { pharmacies: Option[]; users: Option[]; products: Option[] }) {
  ...
  return <form action={action} className="space-y-5"><ActionFeedback {...state}/><div className="grid gap-4 md:grid-cols-2">
    ...
    <Field label="Type"><Select name="missionType" defaultValue="animation"><SelectTrigger className="w-full"><SelectValue/></SelectTrigger><SelectContent>{missionTypes.map((type)=><SelectItem value={type} key={type}>{type}</SelectItem>)}</SelectContent></Select></Field>
    ...
    <Field label="Priorité"><Select name="priority" defaultValue="normal"><SelectTrigger className="w-full"><SelectValue/></SelectTrigger><SelectContent>{["low","normal","high","urgent"].map((v)=><SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent></Select></Field>
    <Field label="Mode"><Select name="locationMode" defaultValue="in_pharmacy"><SelectTrigger className="w-full"><SelectValue/></SelectTrigger><SelectContent>{["in_pharmacy","remote","hybrid","external_event"].map((v)=><SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent></Select></Field>
    ...
    <div className="md:col-span-2"><Field label="Objectif"><Textarea name="objective" required/></Field></div><div className="md:col-span-2"><Field label="Briefing"><Textarea name="briefing" className="min-h-32"/></Field></div>
