...
test("navigation mobile Agent expose les cinq destinations et Plus", async ({ page }) => {
  ...
  await expect(links.nth(1)).toHaveAttribute("href", "/dashboard/agenda");
  await expect(links.nth(2)).toHaveAttribute("href", "/dashboard/pharmacies");
  await expect(links.nth(3)).toHaveAttribute("href", "/dashboard/orders");
